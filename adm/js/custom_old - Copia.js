$(function() {
	...
	$("#btUploadImage").click(function(){
		$('#bannerUploadForm').attr('action','processupload.php').attr('enctype','multipart/form-data').submit();
	});
	...
	$('.widgets.categories .widget .head').click(function(){
		if($(this).find('h5 input').attr('disabled') && !_n_item){
			$(this).toggleClass('inactive').next('.body').fadeToggle(500);
		}
		else{
	...
	var formVarsToAction = function(c,s,b) { 	//Category,Subcategory,Button Action
		if( !c && !s && b===""){
	...
	/* Edit Category */
	$('.widgets.categories .actions .edit_category').click(function(){
		$(this).parent().find('.edit_category, .add_category, .delete_category').hide();
		$(this).parent().find('.save_category').fadeIn(200,function(){
			$(this).click(function(){
				var _t = $(this);
				_t.closest('.head').removeClass('edit').addClass('save');
				_t.hide();
				_t.parent().find('.edit_category, .add_category, .delete_category').fadeIn(200);
				setTimeout(function(){ _t.closest('.head').removeClass('save'); },1500);
				setTimeout(function(){ _t.closest('.head').find('h5 input').attr('disabled',true);},200);
			});
		});
		$(this).fadeOut(200).closest('.head').addClass('edit').find('h5 input').removeAttr('disabled').select();
	});
    ...
    var saveCategory = function(){
		$('.widgets.categories .actions .save_category').click(function(){
		var _t = $(this);
	...
	var _n_item = false;
	$('.widgets.categories .actions .add_category, #categoriesForm > div > .add_category').click(function(e){
		e.preventDefault();
		formVarsToAction();
		e.preventDefault();
		
		var _t = $(this);
		var _w  = jQuery('<div class="widget acc nomargin" />');
		var _html =  '<div class="head">';
			_html += '	<h5><input id="new_subcategory" name="subcategory" placeholder="Defina um título novo" type="text" value=""></h5>';
            _html += '  <div class="actions">';
            _html += '      <a href="javascript:void(0);" class="save_category show" title="Adicionar"><img src="images/ok.png" alt="Adicionar"></a>';
            _html += '      <a href="javascript:void(0);" class="delete_category" title="Excluir"><img src="images/delete.png" alt="Excluir"></a>';
            _html += '  </div>';
			_html += '</div>';
		// Add a Subcategory
		if(  _t.parent().attr('class') === 'actions' ){
			_t.closest('.head').find('a').fadeOut(200);			
			if( !_t.closest('.head').next('.body').is(':visible') ){
				_t.closest('.head').toggleClass('inactive').next('.body').fadeToggle(500);
			}
			_n_item = true;

			//Time to finish the fadeIn
			setTimeout(function(){ 
				if( _t.closest('.head').next('.body').has('div').length ){
					_t.closest('.head').next('.body').find('.widget').prepend(_html);
				}else{
					_w.append(_html);
					_t.closest('.head').next('.body').html(_w.get(0).outerHTML);
				};

				var _t_input = $('#new_subcategory');
				//Delete Prompt
				var _delete = function(elem){
					jConfirm('Você quer realmente excluir este item?', 'Excluir Subcategoria?', function(r) {
						if(r){
							elem.fadeOut(200,function(){
								$(this).remove();
								_n_item = false;
								_t.closest('.head').find('a:not(.save_category)').show();
							})
						}else{
				...
				var _c = false;
				//Delete FocusOut
				_t_input.select().focusout(function(){
					setTimeout(function(){
						if( !_t_input.val().length && !_c ){

							jPrompt('DESISTIU?<br>Digite o título novo no campo abaixo e clique em OK para cadastrar<br>ou em CANCEL para realmente excluir.', '', 'Defina um título novo', function(r) {
								if( r ){
									alert('You entered ' + r);
								}else{
									_t_input.closest('.head').fadeOut(200,function(){
										$(this).remove();
										_n_item = false;
										_t.closest('.head').find('a:not(.save_category)').show();
									})
				...
				categoriesHover(); // Apply Hover Action to New Input
				_t_input.parent().next('.actions').find('.delete_category').click(function(){
					_c = true;
					_delete( $(this).closest('.head') );
				});
				saveCategory();
			},500);
	...
	$("#frm_User, #bannerUploadForm").validationEngine();
