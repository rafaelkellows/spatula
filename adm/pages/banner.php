            ...
            <fieldset>
                <div class="widget first">    
                    ...
                    <div class="rowElem">
                        ...
                        <div class="formRight" style="margin: 5px 12px 0 0">
                            <div id="fileUploader">
                                ...
                                <input name="FileInput[]" id="FileInput" type="file" multiple="multiple" accept="image/*" />
                    ...
                    <div class="rowElem">
                        ...
                        <span style="font-size:11px; display: inline-block; padding: 2px;"><strong>Importante:</strong> não copie e cole um conteúdo diretamente no editor. Isso pode alterar no resultado final.</span>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem">
                        ...
                        <div class="formRight">
                            <select name="link" style="width:525px">
                                <option value="0"> |> Link Externo</option>
                            <?php
                                $oSlct = $oConn->SQLselector("*","produtos","status=1",'id ASC');
                                $bid = ( isset($_GET["bid"] ) ) ? $_GET["bid"] : 0 ;
                                $oBanner = $oConn->SQLselector("*","banners","id='".$bid."'",'');
                                
                                $obPos = strpos($oBanner->fetch(PDO::FETCH_ASSOC)['link'],'id_prod=');
                                $temp = substr($row['link'], $obPos+8);
                                
                                if ($oSlct->rowCount() > 0) {
                                    while ( $row_prod = $oSlct->fetch(PDO::FETCH_ASSOC) ) {
                                        echo '<option value="'.$row_prod['id'].'" '.(($temp == $row_prod['id']) ? 'selected':'').'>'.$row_prod['title'].'</option>';
                                        //echo '<option value="'.$row_prod['id'].'" '.$checked.'>'.$row_prod['title'].' -- '.$obPos.'</option>'; 
                                    }
                                }
                            ?>
                            </select>
                            <span style="font-size:11px; display: inline-block; padding: 2px;"><strong>Dica:</strong> selecione a opção 'Link Externo' para habilitar a opção.</span>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem">
                        ...
                        <div class="formRight">
                            <input type="text" name="external_link" id="external_link" value="<?php print $row['link']; ?>" />
                        </div>
                        <div class="fix"></div>
                    </div>
