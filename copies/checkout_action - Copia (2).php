<?php
  ...
  $senderEmail = ( isset($_REQUEST["senderEmail"] ) ) ? $_REQUEST["senderEmail"] : '' ;
	...
	$url_rqst = $_SERVER['REQUEST_URI'];
	//$exploded = array();
	//parse_str($url_rqst, $exploded);
	...
	for ($x = 1; $x <= $itens; $x++) {
		$xml .=	'        <item>';
		$xml .=	'            <id>itemId'.$x.'</id>';
		$xml .=	'            <description>itemDescription'.$x.'</description>';
		$xml .=	'            <amount>itemAmount'.$x.'</amount>';
		$xml .=	'            <quantity>itemQuantity'.$x.'</quantity>';
		$xml .=	'            <weight>itemWeight'.$x.'</weight>';
		$xml .=	'        </item>';
	} 
	/*for ($x = 1; $x <= $itens; $x++) {
		$xml .=	'        <item>';
		$xml .=	'            <id>'.$_REQUEST['itemId'.$x].'</id>';
		$xml .=	'            <description>'.$_REQUEST['itemDescription'.$x].'</description>';
		$xml .=	'            <amount>'.$_REQUEST['itemAmount'.$x].'</amount>';
		$xml .=	'            <quantity>'.$_REQUEST['itemQuantity'.$x].'</quantity>';
		$xml .=	'            <weight>'.$_REQUEST['itemWeight'.$x].'</weight>';
		$xml .=	'        </item>';
	} */
